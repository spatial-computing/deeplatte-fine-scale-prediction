...
def train(ae, data_obj, args, **kwargs):
    ...
    optimizer = optim.SGD(ae.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-8)
    ...
    for epoch in range(args.epochs):
        ...
        for _, idx in enumerate(idx_data_loader):
            ...
            batch_static_x = np.repeat(data_obj.static_x, len(batch_idx), axis=0)  # (b, n_rows, n_cols, n_features)
            batch_x = np.concatenate([batch_dynamic_x, batch_static_x], axis=-1)
            batch_x = torch.FloatTensor(batch_x).to(kwargs['device'])  # x = (b, t, c, h, w)
